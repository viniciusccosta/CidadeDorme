...
for i in range(qtdBOTS):
	...
	while nome in nomeBOTS:
		nome = input('Nome já existe, digite outro nome: ')
...
while not sair: 											# Enquanto usuário não decidir sair
	...
	while qtdBemVivos > 0 and qtdKillersVivos > 0: 						# A partida continuará até existirem pessoas boas vivas ou até o usuário advinhar todos assassinos
		...
		if usuario.saude == 'Vivo': 										# Usuário só pode influênciar no jogo se ele não morreu
			...
			while nome.lower() not in jogadores or jogadores[nome.lower()].saude == 'Morto':
				print("\n*** OPÇÃO INVÁLIDA ***\n")
				[print('%*s %s' %(tamMaiorNome,jogador.nome,jogador.saude)) for jogador in jogadores.values()]
				print()
				nome 	= input('Detetive, escolha alguém:  ')
			...
			jogador 	= jogadores[ nome.lower() ]
			resposta 	= jogador.responder( perguntas[escolha] )
					
			if nome.lower() not in usuario.jogadoresConhecidos:				# Caso seja a primeira pergunta feita a esse BOT...
				usuario.jogadoresConhecidos[ nome.lower() ] = list()		# 	... criamos uma lista das respostas desse BOT
						
			usuario.jogadoresConhecidos[ nome.lower() ].append(resposta)	# Lista de respostas do BOT
		...
		while escolha != 'n' and escolha != 's':
			...
			if escolha == 's':
				usuario.arriscar(jogadores, partida)
			elif escolha == 'n':
				print("\nOkay")
			else:
				print('\nOpção inválida\n')
...
"""
Objetivos do jogo original:
	
Cidadãos 	devem tentar eliminar 	os assassinos 	através da votação, e também encobrirem os anjos e o detetive para quem os assasinos não os descubram
Assassinos 	devem tentar matar 		a  cidade,    	enganar a cidade para votarem em outras pessoas e tentarem não ser descobertos
Anjos 		devem tentar descobrir 	o  detetive 	para tentar salva-lo, tentarem não serem descobertos e tentarem salvar o máximo de pessoas
Detetive	deve  tentar descobrir 	os assassinos 	e convencer a cidade a votarem neles, além de tentar não ser descoberto

Lembrando que na votação a pessoa é ELIMINADA
Já, os assasinos podem tentar MATAR alguém.
Em ambos casos significa que essa pessoa eliminada/morta deixará de jogar até o fim da partida,
mas uma coisa é uma coisa e outra coisa é outra coisa kkkkkkkk
